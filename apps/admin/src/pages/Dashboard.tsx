...
const ACTIVITY_CONFIG: Record<string, { icon: string; color: string; bgColor: string }> = {
  USER_REGISTERED: { icon: 'person', color: 'text-blue-500', bgColor: 'bg-blue-50' },
  SALON_REGISTERED: { icon: 'storefront', color: 'text-[#006B3F]', bgColor: 'bg-[#006B3F]/10' },
  BOOKING_CREATED: { icon: 'calendar_today', color: 'text-[#B8960F]', bgColor: 'bg-[#FCD116]/20' },
  BOOKING_COMPLETED: { icon: 'check_circle', color: 'text-[#006B3F]', bgColor: 'bg-[#006B3F]/10' },
  BOOKING_CANCELLED: { icon: 'error', color: 'text-[#CE1126]', bgColor: 'bg-[#CE1126]/10' },
  PAYMENT_RECEIVED: { icon: 'credit_card', color: 'text-[#006B3F]', bgColor: 'bg-[#006B3F]/10' },
  SALON_APPROVED: { icon: 'check_circle', color: 'text-[#006B3F]', bgColor: 'bg-[#006B3F]/10' },
  SALON_REJECTED: { icon: 'error', color: 'text-[#CE1126]', bgColor: 'bg-[#CE1126]/10' },
  REVIEW_SUBMITTED: { icon: 'content_cut', color: 'text-blue-500', bgColor: 'bg-blue-50' },
  default: { icon: 'schedule', color: 'text-gray-500', bgColor: 'bg-gray-50' },
};
...
function StatCardSkeleton() {
  return (
    <div className="bg-white rounded-xl shadow-sm p-3 sm:p-5 border-l-4 border-gray-200">
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <div className="skeleton-shimmer w-20 h-4 mb-2" />
          <div className="skeleton-shimmer w-24 h-8 sm:h-10 mb-2" />
...
function ChartSkeleton({ height = 288 }: { height?: number }) {
  return (
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
      <div className="skeleton-shimmer w-32 h-5 mb-4" />
      ...
      <div className="space-y-4">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="skeleton-shimmer w-full h-4 mb-1" />
              <div className="skeleton-shimmer w-24 h-3" />
            </div>
            <div className="skeleton-shimmer w-12 h-4 rounded-full flex-shrink-0" />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="skeleton-shimmer w-40 h-5" />
        <div className="skeleton-shimmer w-8 h-8 rounded-lg" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 mb-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="p-3 sm:p-4 rounded-lg border border-gray-100">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                <div className="skeleton-shimmer w-8 h-8 rounded-lg" />
                <div>
                  <div className="skeleton-shimmer w-20 h-3 mb-1" />
                  <div className="skeleton-shimmer w-16 h-4" />
                </div>
              </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="skeleton-shimmer w-8 h-4" />
        </div>
        <div className="skeleton-shimmer w-full h-2.5 rounded-full" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <div className="p-3 sm:p-4 rounded-lg border border-gray-100">
          <div className="skeleton-shimmer w-32 h-4 mb-2" />
          <div className="skeleton-shimmer w-16 h-8" />
        </div>
        <div className="p-3 sm:p-4 rounded-lg border border-gray-100">
          <div className="skeleton-shimmer w-32 h-4 mb-2" />
          <div className="skeleton-shimmer w-16 h-8" />
    ...
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 card-v2">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base sm:text-lg font-semibold text-gray-800 flex items-center gap-2">
          <Icon name="monitoring" className="w-5 h-5 text-[#006B3F]" />
          System Monitoring
        </h2>
        <div className="flex items-center gap-2">
          {isFetching && <Icon name="progress_activity" className="animate-spin text-gray-400" size={16} />}
          <button
            onClick={() => refetch()}
            className="btn-ripple p-2 text-gray-500 hover:text-[#006B3F] hover:bg-[#006B3F]/10 rounded-lg transition-colors"
            title="Refresh now"
          >
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <div className={`p-2 rounded-lg ${card.isHealthy ? 'bg-green-100' : 'bg-red-100'}`}>
                    <Icon name={card.icon} className={`w-4 h-4 ${card.isHealthy ? 'text-green-600' : 'text-red-600'}`} />
                  </div>
                  <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        <div className="p-3 sm:p-4 rounded-lg border border-blue-100 bg-blue-50">
          <div className="flex items-center gap-2 mb-2">
            <Icon name="group" className="w-4 h-4 text-blue-600" />
        ...
        <div className={`p-3 sm:p-4 rounded-lg border ${suspiciousActivity.recentFailedLogins > 0 ? 'border-red-200 bg-red-50' : 'border-green-100 bg-green-50'}`}>
          <div className="flex items-center gap-2 mb-2">
            <Icon name="security" className={`w-4 h-4 ${suspiciousActivity.recentFailedLogins > 0 ? 'text-red-600' : 'text-green-600'}`} />
            ...
            <div key={activity.id} className="flex items-center justify-between py-2 sm:py-3 border-b border-gray-100 last:border-0 last:pb-0">
              <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 ${config.bgColor}`}>
                  <Icon name={config.icon} className={`w-4 h-4 sm:w-5 sm:h-5 ${config.color}`} />
                </div>
  ...
  const statsCards = [
    {
      ...
      gradient: 'from-[#006B3F]/20 to-transparent',
    },
    {
      ...
      subtitle: 'last 24h',
      gradient: 'from-[#FCD116]/20 to-transparent',
    },
    {
      title: 'Total Users',
    ...
    },
    {
      ...
      gradient: 'from-[#CE1126]/20 to-transparent',
    },
  ];
    ...
    <div className="space-y-6 page-enter">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Here's what's happening on your platform.</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto justify-start">
          {[
            ...
            <button
              ...
              className={`btn-ripple flex-1 sm:flex-none px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium rounded-full transition-all duration-200 min-h-[44px] ${
                period === option.value
      ...
      {/* Stats Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 fade-section">
        {statsLoading ? (
          <>
            <StatCardSkeleton />
            <StatCardSkeleton />
          ...
          statsCards.map((stat) => {
            return (
              <div 
                key={stat.title} 
                className={`card-v2 p-3 sm:p-5 border-l-4 ${stat.borderColor} relative overflow-hidden`}
              >
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center mt-1 sm:mt-2 gap-1 flex-wrap">
                      {stat.trend === 'up' && <Icon name="trending_up" size={12} className="text-green-500 sm:w-3.5 sm:h-3.5" />}
                      {stat.trend === 'down' && <Icon name="trending_down" size={12} className="text-red-500 sm:w-3.5 sm:h-3.5" />}
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-3 sm:p-4 fade-section">
        <h2 className="text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center gap-2">
          <Icon name="account_balance_wallet" className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500" />
          Financial Overview
        </h2>
      ...
      {/* Charts Row 1 */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 fade-section">
        {/* Bookings Chart */}
        {metricsLoading ? (
          <ChartSkeleton height={288} />
        ) : (
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 card-v2 relative overflow-hidden">
            {/* Gradient top border accent - blue for bookings */}
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#FCD116]/40 to-transparent" />
        ...
        {metricsLoading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 card-v2 relative overflow-hidden">
            {/* Gradient top border accent - green for revenue */}
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#006B3F]/40 to-transparent" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 fade-section">
        ...
        {metricsLoading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 card-v2 relative overflow-hidden">
            {/* Gradient top border accent - blue for user growth */}
        ...
        {statsLoading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 card-v2 relative overflow-hidden">
            {/* Gradient top border accent - multi-color for salon status */}
            <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#006B3F]/40 via-[#FCD116]/40 to-[#CE1126]/40" />
