...
// ============================================================
// GroomLink Chat Widget Hook (Customer - authenticated users)
// ============================================================
// Hook that manages chat session for logged-in customers.
// - Uses authenticated API endpoints (/api/me/support/tickets)
// - Connects to Socket.io for real-time messages
...
export interface UseCustomerChatReturn {
  open: boolean;
...
export function useCustomerChat(): UseCustomerChatReturn {
    ...
    const fetchMessages = async () => {
      try {
        ...
        if (!cancelled && json.success && Array.isArray(json.data?.messages)) {
          ...
          setMessages((prev) => {
            // Bump unread if new agent messages arrived while closed
