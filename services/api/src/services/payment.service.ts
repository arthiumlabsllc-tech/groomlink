...
export async function initializePayment(
  ...
  // Resolve the payment provider via the capability-aware smart router.
  // For mobile money (the booking flow's only payment method today), this:
  //   - prefers the admin-selected active gateway when it's registered,
  //   - falls back to ANY mobile-money-capable provider if the active gateway
  //     isn't configured (e.g. admin selected Hubtel but credentials are missing).
  // Result: while only Paystack is configured, mobile money always routes to
  // Paystack regardless of what the gateway dropdown is set to. When Hubtel
  // credentials are added later, the same dropdown selection takes effect with
  // zero code changes.
  const activeProvider = await paymentProviderRegistry.getProviderForPaymentMethod('mobile_money');
...
/**
 * Handle TheTeller webhook
 * TheTeller sends webhook with transaction status
 */
export async function handleTheTellerWebhook(
  rawBody: string,
  headers: Record<string, string>
): Promise<{ success: boolean; message: string }> {
  try {
    ...
    // TheTeller may send as JSON or URL-encoded data
    ...
    if (!transactionId) {
      logger.error('TheTeller webhook missing transaction_id', { rawBody });
      return { success: false, message: 'Missing transaction_id' };
    }

    logger.info('TheTeller webhook event received', {
      transactionId,
      status,
      code,
      reason,
    ...
    if (!thetellerApiKey || !thetellerApiUser || !thetellerMerchantId) {
      logger.error('TheTeller credentials not configured');
      return { success: false, message: 'TheTeller not configured' };
    }
    ...
    // Import TheTeller provider
    const { TheTellerProvider } = await import('./theteller.provider');
    const theteller = new TheTellerProvider();

    // Handle the webhook
    const webhookResponse = await theteller.handleWebhook(
      { rawBody, headers },
      credentials
    );

    if (!webhookResponse.success) {
      logger.error('TheTeller webhook processing failed', {
        transactionId,
        message: webhookResponse.message,
      });
      return { success: false, message: webhookResponse.message };
    }

    // If payment was successful, update the payment record
    if (webhookResponse.status === 'success' && webhookResponse.transactionReference) {
      const reference = webhookResponse.transactionReference;
      ...
      if (!payment) {
        logger.warn('Payment not found for TheTeller webhook', { reference });
        return { success: false, message: 'Payment not found' };
      }

      if (payment.status === PaymentStatus.SUCCESS) {
        logger.info('TheTeller payment already completed', { reference });
        return { success: true, message: 'Payment already processed' };
      }

      // Complete payment using existing function
      const result = await verifyAndCompletePayment(payment.id, reference);
      
      if (result.success) {
        logger.info('TheTeller payment completed via webhook', {
          reference,
          bookingId: payment.bookingId,
        });
      } else {
        logger.error('Failed to complete TheTeller payment', {
          reference,
          message: result.message,
        });
      }
